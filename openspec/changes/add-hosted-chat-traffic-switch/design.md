...
- 保持本地 `.env.local`、其他托管平台和 `CHAT_ENABLED` 缺省部署向后兼容。
...
- 未配置：启用，保持现有部署兼容。
- 去除空白后大小写不敏感的 `true`：启用。
- 显式 `false` 或其他已配置值：禁用，错误配置时 fail closed。
...
- `CHAT_ENABLED` 缺省启用 → 保持兼容，但新部署需要操作者主动设置；README 推荐 Production 显式配置。
...
1. 新代码在 `CHAT_ENABLED` 缺省时保持 Chat 启用。
2. 首次部署后在 Vercel Production 显式添加 `CHAT_ENABLED=true` 并 Redeploy。
