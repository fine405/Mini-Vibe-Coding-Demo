...
- 将 Header 中现有单按钮 Theme Toggle 替换为独立的 `Theme` 下拉菜单，保持在 Header 一级，与 `More` 菜单分离。
...
用户已于 2026-07-18 指定 Summer 暂时直接使用 dany.works 的叶影视频，并补充要求保留原站森林环境声；随后明确批准开始实现本 proposal。
